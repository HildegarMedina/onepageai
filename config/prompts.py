CREATE_ONE_PAGE = """
    ...
    Incluye: Header, {sections} y un pie de página simple con el año actual. No olvides resetear los estilos del navegador con normalize, y debes importarlo antes de cualquier otro CSS. Si el pie de pagina tiene una etiqueta p, asegurate que tenga margin-bottom: 0, para que no quede espacio extra.

    Para el header, si usas un framework, usa la misma estructura del navbar del framework, para que sea 100% responsive. Usa imágenes de relleno usando esta url: https://placehold.co/200x200, cambia los valores de 200x200 por la resolución que creas conveniente.

    Usa CDN, no uses ninguna url que contenga 'stackpath', además intenta dar estilos a todos los elementos, usando framework en caso se te haya indicado.
    ...
    Los títulos deben estar en bold y usa Roboto de Google Fonts. Todo título debe tener margin-bottom: 2rem y margin-top: 1rem al menos. Los h3 siempre deben tener un margin-top: 1rem si tienen una imagen anterior.
